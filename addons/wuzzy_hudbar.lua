...
local bar_id = "fatique"
local BAR_BGICON = "blank.png"         -- Change to your background icon if needed
local BAR_ICON = "server_favorite.png" -- Change to your bar image (2x16) if needed

-- Register the fatique HUD bar
hb.register_hudbar(
    bar_id,
    0xFFFFFF, -- White text
    "Fatique", -- Label
...
-- Helper functions to get/set fatique values
local function get_fatique(player)
    local state = hb.get_hudbar_state(player, bar_id)
    return state and state.value or 0, state and state.max or 20
end

local function set_fatique(player, value, max)
...
aio_double_tap_run.register_callback(function(player, data, dtime)
    ...
    local value, max_value = get_fatique(player)
    ...
    if value <= 1 then
        ...
        set_fatique(player, 0, max_value)
        return data
    end
    ...
    if data.is_sprinting and not functions.in_air(player, 2) and not functions.wall_bump(player) then
        ...
        if value > 0 then
            set_fatique(player, math.max(0, value - 0.5 * dtime), max_value)
        end
    else
        ...
        if sprint_timer[player_name] >= 2 and player_health > health_threshold then
            if value < max_value then
                set_fatique(player, math.min(max_value, value + 1 * dtime), max_value)
            end
        end
    end
end)
